...
Sync configuration across devices via WebDAV or S3-compatible storage. The client now writes **protocol v3** with eleven independently selectable categories (A–K). All categories are selected by default, matching the previous full-snapshot intent. Turning a category off stops this device from uploading or downloading it; existing local and remote data are kept.

Provider, MCP, common-config, and proxy settings may contain API keys or tokens. Skill files (F) require the Skill inventory (E); with F off the client will not walk, zip, or transfer the Skill directory.

The new client can still read a legacy v2 snapshot and restore only the selected categories, but it only writes v3. Every participating device must upgrade. A legacy full snapshot can be deleted only after a valid v3 snapshot exists, via **Manage cloud data**.
...
| Auto Sync | When enabled, local changes to selected, ready categories are debounced and uploaded (never auto-downloaded) |
| Sync content | Eleven A–K checkboxes shared by S3 and WebDAV; all on by default. Turning an item off does not delete local or remote data |

#### Operations

| Action | Description |
|--------|-------------|
| **Test Connection** | Verify the server URL and credentials |
| **Upload** | Upload only selected, ready categories; disabled or pending items are skipped |
| **Download** | Replace only selected categories. An explicit empty remote category clears local data; a missing remote category does not |
| **Manage cloud data** | Delete v3 categories disabled on this device, or delete the legacy full snapshot after a valid v3 snapshot exists |

#### Auto-Sync

When **Auto Sync** is enabled:
- A confirmation dialog is shown on first activation
- Only changes mapped to selected, `ready` categories trigger an upload
- With every category off, connection settings are kept but no sync runs

#### Remote Snapshot Info

Before upload or download, CC Switch shows the scope of this operation:
- Protocol version (v3 first; **legacy combined data is shown only when a v2 snapshot actually exists** — an empty remote is not labeled as legacy)
- Database compatibility version
- Timestamp and device name
- Categories included, skipped, and estimated transfer size
- Sensitive-data notice (provider, MCP, common-config, and proxy settings may contain secrets)

> **Note**: Upload overwrites the **selected remote categories**; download overwrites the **selected local categories**. Unselected categories stay unchanged. Every participating device must upgrade.
