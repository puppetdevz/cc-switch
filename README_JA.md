...
- **クラウド同期** -- Dropbox、OneDrive、iCloud、WebDAV、または S3 互換ストレージで、カテゴリを選んで設定を同期
...
- **クラウド同期** -- カスタム設定ディレクトリ（Dropbox、OneDrive、iCloud、NAS）に加え、WebDAV / S3 互換同期。A–K カテゴリを選択可能（既定はすべてオン）。オフにしてもローカル/クラウドのデータは削除しません。プロバイダー/MCP/共通設定/プロキシには機密情報が含まれる場合があります。新クライアントは v3 のみ書き込み、旧 v2 スナップショットは選択的に読み取れます
