...
fn merge_enabled_prompt_content<'a, I>(enabled: I) -> String
where
...
fn project_prompt_set_to_path(
    ...
    let enabled: Vec<&Prompt> = prompts.values().filter(|prompt| prompt.enabled).collect();
    ...
    write_text_file(target_path, &merge_enabled_prompt_content(enabled))?;
    Ok(None)
}
...
impl PromptService {
    ...
    pub fn sync_merged_prompts_to_file(state: &AppState, app: AppType) -> Result<(), AppError> {
        ...
        let merged_content =
            merge_enabled_prompt_content(prompts.values().filter(|prompt| prompt.enabled));
